...
${truncatedReasoning}`);
  for (const r of searchResults) {
    ...
    if (r.agentId === "chatluna-search") {
      const summaryEndIndex = r.findings.indexOf(SUMMARY_SEPARATOR);
      if (summaryEndIndex !== -1) {
        cleanFindings = r.findings.substring(0, summaryEndIndex + SUMMARY_SEPARATOR.length) + "\n\n(搜索详情已在合并消息中省略，请查看判决依据)";
      }
    }
...
var VerifyAgent = class {
  ...
  compactSearchResults(searchResults) {
    const maxFindingsChars = 800;
    return searchResults.map((result) => {
      let findings = result.findings || "";
      if (result.agentId === "chatluna-search") {
        const summaryEndIndex = findings.indexOf(SUMMARY_SEPARATOR);
        if (summaryEndIndex !== -1) {
          findings = findings.substring(0, summaryEndIndex + SUMMARY_SEPARATOR.length) + "\n\n(搜索详情已省略)";
        }
      }
