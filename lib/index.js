...
⏱️ ${(processingTime / 1e3).toFixed(1)}秒`;
  const details = [];
  const truncatedReasoning = reasoning.length > MAX_SEGMENT_LENGTH ? reasoning.substring(0, MAX_SEGMENT_LENGTH) + "..." : reasoning;
  details.push(`📝 判决依据
...
${truncatedReasoning}`);
  for (const r of searchResults) {
    ...
    const truncatedFindings = cleanFindings.length > MAX_SEGMENT_LENGTH ? cleanFindings.substring(0, MAX_SEGMENT_LENGTH) + "..." : cleanFindings;
