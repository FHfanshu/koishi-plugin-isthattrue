    ...
    async search(query) {
        ...
        try {
            ...
            const searchPromises = queries.map(async (q) => {
                return (0, async_1.withTimeout)((async () => {
                    ...
                    try {
                        ...
                        const searchData = (0, search_1.normalizeResultItems)(searchResult)
                        ...
                        try {
                            ...
                            const retryData = (0, search_1.normalizeResultItems)(retryResult)
                                .slice(0, MAX_RESULTS_PER_QUERY);
            ...
            const formattedResults = finalSearchData.length > 0
                ? finalSearchData.map((item, i) => `[${i + 1}] ${(0, text_1.truncate)(item.title || '未知标题', 120)}\n来源: ${item.url || '未知'}\n${(0, text_1.truncate)(item.description || item.content || '', MAX_DESC_LENGTH)}`).join('\n\n---\n\n')
                : '未找到搜索结果';
            // 添加统计摘要
