...
class FactCheckTool extends tools_1.Tool {
    ...
    createProviderTask(subSearchAgent, claim, provider, index) {
    ...
    async _call(input) {
        ...
        try {
            ...
            if (!this.config.agent.enableMultiSourceSearch || providers.length === 1) {
                ...
                const result = await this.withTimeout(subSearchAgent.deepSearchWithModel(claim, provider.model, `tool-${provider.key}`, provider.label, (0, prompts_1.buildFactCheckToolSearchPrompt)(claim), prompts_1.FACT_CHECK_TOOL_SEARCH_SYSTEM_PROMPT), timeout, provider.label);
                if (result.failed) {
            ...
            if (failedLabels.length > 0) {
                return this.appendContext(`${output}\n\n[Failed]\n- ${failedLabels.join('\n- ')}`, context);
            }
            return this.appendContext(output, context);
        }
        catch (error) {
