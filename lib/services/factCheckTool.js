    ...
    async _callInner(rawClaim) {
        ...
        try {
            ...
            if (!this.config.factCheck.enableMultiSourceSearch || providers.length === 1) {
                ...
                const result = await (0, async_1.withTimeout)(this.subSearchAgent.deepSearchWithModel(claim, provider.model, `tool-${provider.key}`, provider.label, (0, prompts_1.buildFactCheckToolSearchPrompt)(claim), prompts_1.FACT_CHECK_TOOL_SEARCH_SYSTEM_PROMPT), this.config.factCheck.perSourceTimeout, provider.label);
                if (result.failed) {
