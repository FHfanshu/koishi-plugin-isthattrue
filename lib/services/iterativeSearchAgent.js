...
class IterativeSearchAgent {
    ...
    async search(query) {
        ...
        if (query.useTool === 'ollama_search' && (0, apiConfig_1.hasEnabledApiProvider)(this.config, 'ollama')) {
            try {
...
3. 结果简明，不要输出与任务无关说明`;
    ...
    getEnabledProviders() {
        const providers = [];
        if (this.config.deepSearch.grokModel?.trim())
            providers.push('grok');
        if (this.config.deepSearch.geminiModel?.trim())
            providers.push('gemini');
        if (this.config.deepSearch.chatgptModel?.trim())
