...
const searxngSearch_1 = require("./searxngSearch");
...
class IterativeSearchAgent {
    ...
    constructor(ctx, config) {
        ...
        this.searXNGSearchService = new searxngSearch_1.SearXNGSearchService(ctx, config);
    ...
    async search(query) {
        ...
        if (shouldUseSearXNG) {
            try {
                ...
                this.logger.warn(`[IterativeSearch] SearXNG 调用失败，回退模型搜索: ${error.message}`);
            }
        }
...
3. 结果简明，不要输出与任务无关说明`;
    ...
    getEnabledProviders() {
        ...
        if (this.config.deepSearch.searchUseDeepseek)
