...
class OllamaSearchService {
    ...
    getSettings(scope) {
        return {
            apiBase: (0, apiConfig_1.resolveOllamaApiBase)(this.config, scope),
            apiKey: (0, apiConfig_1.resolveOllamaApiKey)(this.config, scope),
            maxResults: Math.max(1, Math.min(this.config.factCheck.ollamaSearchMaxResults || 5, 10)),
            timeout: Math.max(3000, Math.min(this.config.factCheck.ollamaSearchTimeout || 15000, 120000)),
        };
    }
