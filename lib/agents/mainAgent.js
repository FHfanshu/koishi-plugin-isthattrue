...
class MainAgent {
    ...
    constructor(ctx, config) {
        ...
        this.tavilySearchAgent = new tavily_1.TavilySearchAgent(ctx, config);
    ...
    async verify(content) {
        ...
        try {
            ...
            const finalResult = await this.verifyAgent.verify(content, processedResults, imageBase64List);
            return {
                ...finalResult,
                processingTime: Date.now() - startTime,
            };
    ...
    async searchEvidenceLegacy(searchText) {
        ...
        if (this.tavilySearchAgent.isAvailable()) {
            searchTasks.push({
                name: 'TavilySearch',
                promise: this.withTimeout(this.tavilySearchAgent.search(searchText), this.config.tof.timeout, 'TavilySearch')
            });
        }
