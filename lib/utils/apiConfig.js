...
exports.resolveSearXNGApiBase = resolveSearXNGApiBase;
...
function getEnabledApiEntry(config, provider) {
...
function resolveSearXNGApiBase(config) {
    const entry = getEnabledApiEntry(config, 'searxng');
    return pick(config.deepSearch.searXNGApiBase, entry?.baseUrl, config.api.searXNGApiBase, DEFAULT_SEARXNG_API_BASE);
}
