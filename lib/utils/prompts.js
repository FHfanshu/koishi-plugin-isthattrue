...
exports.IMAGE_DESCRIPTION_PROMPT = exports.OCR_PROMPT = exports.VERIFY_AGENT_SYSTEM_PROMPT_MULTIMODAL = exports.VERIFY_AGENT_SYSTEM_PROMPT = exports.DEEP_SEARCH_SYNTHESIZE_SYSTEM_PROMPT = exports.DEEP_SEARCH_EVALUATE_SYSTEM_PROMPT = exports.DEEP_SEARCH_CONTROLLER_SYSTEM_PROMPT = exports.FACT_CHECK_TOOL_SEARCH_SYSTEM_PROMPT = exports.DEEP_SEARCH_AGENT_SYSTEM_PROMPT = exports.SEARCH_AGENT_SYSTEM_PROMPT = exports.VERDICT_EMOJI = void 0;
exports.buildSubSearchPrompt = buildSubSearchPrompt;
exports.buildFactCheckToolSearchPrompt = buildFactCheckToolSearchPrompt;
exports.buildDeepSearchPlanPrompt = buildDeepSearchPlanPrompt;
exports.buildDeepSearchEvaluatePrompt = buildDeepSearchEvaluatePrompt;
exports.buildDeepSearchSynthesizePrompt = buildDeepSearchSynthesizePrompt;
exports.buildVerifyPrompt = buildVerifyPrompt;
exports.formatVerificationOutput = formatVerificationOutput;
exports.formatForwardMessages = formatForwardMessages;
 ...
 */
exports.SEARCH_AGENT_SYSTEM_PROMPT = `你是事实核查搜索员。搜索验证声明的相关信息。
 ...
 */
exports.DEEP_SEARCH_AGENT_SYSTEM_PROMPT = `你是事实核查搜索员，专门使用 X (Twitter) 和网络搜索验证声明。
 ...
 */
exports.FACT_CHECK_TOOL_SEARCH_SYSTEM_PROMPT = `你是事实核查搜索员，专门使用 X (Twitter) 和网络搜索核查待验证内容。
 ...
 * DeepSearch 主控系统提示词
 */
exports.DEEP_SEARCH_CONTROLLER_SYSTEM_PROMPT = `你是 DeepSearch 主控模型，负责规划和协调多轮迭代搜索。
 ...
 * DeepSearch 评估系统提示词
 */
exports.DEEP_SEARCH_EVALUATE_SYSTEM_PROMPT = `你是 DeepSearch 评估模型，负责判断当前搜索结果是否足够支撑结论。
 ...
 * DeepSearch 综合系统提示词
 */
exports.DEEP_SEARCH_SYNTHESIZE_SYSTEM_PROMPT = `你是 DeepSearch 综合报告模型。
 ...
 */
exports.VERIFY_AGENT_SYSTEM_PROMPT = `你是事实核查裁判。基于搜索证据做出判决。
 ...
 */
exports.VERIFY_AGENT_SYSTEM_PROMPT_MULTIMODAL = `你是事实核查裁判。基于搜索证据和图片内容做出判决。
 ...
 * 图片 OCR 提示词
 */
exports.OCR_PROMPT = `提取图片中的文字。无文字则简述图片内容。`;
/**
 * 图片描述提取提示词
 * 用于纯图片输入时，提取内容供搜索使用
 */
exports.IMAGE_DESCRIPTION_PROMPT = `请仔细观察这张图片，描述其中的主要内容。
 ...
 */
function buildDeepSearchPlanPrompt(claim, history) {
    return `待验证声明：
"${claim}"
 ...
 */
function buildDeepSearchEvaluatePrompt(claim, results, history) {
    return `待验证声明：
"${claim}"
 ...
 */
function buildDeepSearchSynthesizePrompt(claim, history) {
    return `待验证声明：
"${claim}"
...
function formatVerificationOutput(content, searchResults, verdict, reasoning, sources, confidence, processingTime, format = 'markdown') {
    const verdictLabel = exports.VERDICT_EMOJI[verdict] || verdict;
    ...
    if (format === 'plain') {
        ...
        if (sources.length > 0) {
            output += `\n源：\n`;
            output += sources.map(s => `• ${(0, url_1.removeCensorshipBypass)(s)}`).join('\n');
            output += `\n`;
        }
...
`;
    if (sources.length > 0) {
        output += `
🔗 **参考来源:**
${sources.map(s => `• ${(0, url_1.removeCensorshipBypass)(s)}`).join('\n')}
`;
    }
    output += `
⏱️ *处理耗时：${(processingTime / 1000).toFixed(1)}秒*`;
 ...
 */
function formatForwardMessages(content, searchResults, verdict, reasoning, sources, confidence, processingTime, maxSegmentLength = OUTPUT.SEGMENT_MAX_CHARS) {
    const confidenceValue = Math.round(clampConfidence(confidence) * 100);
    const verdictLabel = exports.VERDICT_EMOJI[verdict] || verdict;
    // 主消息：简要判决
    ...
    if (sources.length > 0) {
        ...
        const sourcesText = limitedSources
            .map(s => `• ${(0, url_1.removeCensorshipBypass)(s).substring(0, OUTPUT.CONTENT_SUMMARY_MAX)}`)
            .join('\n');
