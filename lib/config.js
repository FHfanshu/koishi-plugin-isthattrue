...
const factCheckDebugSchema = koishi_1.Schema.object({
    timeout: koishi_1.Schema.number().min(10000).max(300000).default(60000).description('单次请求超时时间 (毫秒)'),
...
const agentToolSchema = koishi_1.Schema.object({
    ...
    quickToolModel: koishi_1.Schema.dynamic('model').default('').description('Gemini 快速搜索模型（留空时回退 geminiModel）'),
    quickToolTimeout: koishi_1.Schema.number().min(3000).max(120000).default(15000).description('Gemini 快速搜索超时（毫秒）'),
...
const agentSearchSchema = koishi_1.Schema.object({
    ...
    grokModel: koishi_1.Schema.dynamic('model').default('x-ai/grok-4-1').description('Grok 来源模型（留空则跳过 Grok 来源）'),
    geminiModel: koishi_1.Schema.dynamic('model').default('').description('Gemini 来源模型（留空则跳过 Gemini 来源）'),
    chatgptModel: koishi_1.Schema.dynamic('model').default('').description('ChatGPT 来源模型（留空则跳过 ChatGPT 来源）'),
    ollamaSearchMaxResults: koishi_1.Schema.number().min(1).max(10).default(5).description('Ollama Search 返回结果数'),
    ollamaSearchTimeout: koishi_1.Schema.number().min(3000).max(120000).default(15000).description('Ollama Search 超时（毫秒）'),
    ...
    chatlunaSearchContextTimeout: koishi_1.Schema.number().min(3000).max(120000).default(12000).description('Chatluna Search 上下文查询超时（毫秒）；超时仅跳过附加上下文'),
    chatlunaSearchContextMaxChars: koishi_1.Schema.number().min(200).max(8000).default(1200).description('附加的 Chatluna Search 上下文最大字符数（防止输出过长）'),
    chatlunaSearchContextMaxSources: koishi_1.Schema.number().min(1).max(20).default(5).description('附加的 Chatluna Search 来源数量上限'),
    ...
    ollamaSearchContextTimeout: koishi_1.Schema.number().min(3000).max(120000).default(12000).description('Ollama Search 上下文查询超时（毫秒）；超时仅跳过附加上下文'),
    ollamaSearchContextMaxChars: koishi_1.Schema.number().min(200).max(8000).default(1200).description('附加的 Ollama Search 上下文最大字符数（防止输出过长）'),
    ollamaSearchContextMaxSources: koishi_1.Schema.number().min(1).max(20).default(5).description('附加的 Ollama Search 来源数量上限'),
}).description('搜索配置');
...
const deepSearchLLMSourceSchema = koishi_1.Schema.object({
    grokModel: koishi_1.Schema.dynamic('model').default('x-ai/grok-4-1').description('Grok 模型（留空则跳过 Grok 源）'),
    geminiModel: koishi_1.Schema.dynamic('model').default('').description('Gemini 模型（留空则跳过 Gemini 源）'),
    chatgptModel: koishi_1.Schema.dynamic('model').default('').description('ChatGPT 模型（留空则跳过 ChatGPT 源）'),
    ollamaSearchMaxResults: koishi_1.Schema.number().min(1).max(10).default(5).description('Ollama Search 返回结果数'),
    ollamaSearchTimeout: koishi_1.Schema.number().min(3000).max(120000).default(15000).description('Ollama Search 超时（毫秒）'),
}).description('搜索源');
...
exports.Config = koishi_1.Schema.intersect([
    koishi_1.Schema.object({
        ...
        deepSearch: koishi_1.Schema.intersect([deepSearchCoreSchema, deepSearchLLMSourceSchema, deepSearchChatlunaIntegrationSchema]).description('DeepSearch'),
