...
export class DeepSearchController {
  private logger
  private searchAgent: IterativeSearchAgent
  private chatlunaAdapter: ChatlunaAdapter
  ...
  async search(claim: string): Promise<DeepSearchReport> {
    ...
    for (let i = 0; i < maxIterations; i++) {
      ...
      try {
        ...
        this.logger.warn(`[DeepSearch] 第 ${roundNumber} 轮失败: ${(error as Error).message}`)
        history.rounds.push({
          round: roundNumber,
          plan: {
          ...
          results: [{
            ...
            findings: `执行失败: ${(error as Error).message}`,
            sources: [],
            confidence: 0,
            failed: true,
          ...
          evaluation: {
            shouldStop: true,
            reason: `轮次异常终止: ${(error as Error).message}`,
            confidence: 0,
          },
          elapsedMs: Date.now() - roundStart,
        })
    ...
    try {
      ...
      this.logger.warn(`[DeepSearch] 综合报告生成失败，回退本地汇总: ${(error as Error).message}`)
      return this.buildFallbackReport(history)
    }
  }

  async plan(claim: string, history?: DeepSearchHistory): Promise<DeepSearchPlan> {
  ...
  private async executePlan(plan: DeepSearchPlan): Promise<SearchResult[]> {
    ...
    return settled.map((item, index) => {
      ...
      const message = (item.reason as Error)?.message || String(item.reason)
      return {
  ...
  private parseEvaluation(content: string, results: SearchResult[]): DeepSearchEvaluation {
    const parsed = this.parseJson(content)
    if (!parsed) {
    ...
    return {
      ...
      gaps: Array.isArray(parsed.gaps)
        ? parsed.gaps.map((item: any) => String(item).trim()).filter(Boolean)
        : undefined,
    }
  }
  ...
  private parseFinalReport(content: string, history: DeepSearchHistory): DeepSearchReport {
    ...
    const sources = Array.isArray(parsed.sources)
      ? parsed.sources.map((item: any) => String(item).trim()).filter(Boolean)
      : this.collectAllSources(history)

    const keyFindings = Array.isArray(parsed.keyFindings)
      ? parsed.keyFindings.map((item: any) => String(item).trim()).filter(Boolean)
  ...
  private buildFallbackEvaluation(results: SearchResult[]): DeepSearchEvaluation {
  ...
  private estimateConfidence(results: SearchResult[]): number {
